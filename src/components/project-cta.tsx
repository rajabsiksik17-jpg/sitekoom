  ...
  return (
    <section className="container-site py-10 sm:py-20">
      <div className="grid gap-4 lg:grid-cols-2 sm:gap-6">
        <div className="relative overflow-hidden rounded-3xl bg-ink-900 p-5 text-white sm:p-12">
          <div className="absolute inset-0 bg-hero-gradient" />
          <div className="relative">
            <h2 className="text-xl font-extrabold sm:text-3xl lg:text-4xl">{isAr ? "لديك فكرة أو مشروع جديد؟" : "Have a project in mind?"}</h2>
            <p className="mt-2 max-w-md text-sm text-white/70 sm:mt-4 sm:text-base">
              {isAr
        ...
        <div className="card p-5 sm:p-8">
          <div className="mb-3 flex items-center gap-3 sm:mb-5">
            {settings.logo ? (
              ...
              <img src={settings.logo} alt={isAr ? settings.company_name_ar : settings.company_name_en} className="h-9 w-auto sm:h-12" />
            ) : (
          ...
          <h3 className="text-lg font-extrabold text-ink-900 sm:text-2xl">{isAr ? "نحن هنا لمساعدتك" : "We're Here to Help"}</h3>
