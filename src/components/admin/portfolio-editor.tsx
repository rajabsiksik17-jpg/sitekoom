...
import { Plus, Trash2, ChevronUp, ChevronDown, Eye, EyeOff, Star } from "lucide-react";
...
const emptyItem = (type: string): PortfolioItemDraft => ({
  type,
  ...
  is_visible: true, is_featured: false, sort: 0, data: {},
});

function isImageKind(kind: PortfolioKind) {
  return kind === "image";
}
function isLinkKind(kind: PortfolioKind) {
  return kind === "link";
}
...
export function PortfolioEditor({
  ...
  function addItem(type: string) {
    ...
    item.sort = value.filter((i) => i.type === type).length;
    onChange([...value, item]);
  }
  ...
  return (
    <div className="space-y-6">
      {enabledTypes.map((type) => {
        const items = value
          .map((it, i) => ({ it, i }))
          .filter(({ it }) => it.type === type.key);
        return (
          <div key={type.key} className="rounded-xl border border-brand-100 p-4">
            <div className="mb-3 flex items-center justify-between">
              <p className="font-bold text-ink-900">{type.labelAr}</p>
              <button type="button" onClick={() => addItem(type.key)} className="btn-secondary px-3 py-1.5 text-xs">
                <Plus className="h-3.5 w-3.5" /> إضافة
              </button>
            </div>

            {items.length === 0 ? (
              <p className="py-3 text-center text-xs text-gray-400">لا توجد عناصر بعد.</p>
            ) : (
              <div className="space-y-3">
                {items.map(({ it, i }) => (
                  <ItemCard key={i} item={it} index={i} total={value.length} onUpdate={(p) => updateItem(i, p)} onRemove={() => removeItem(i)} onMove={(d) => moveItem(i, d)} />
                ))}
              </div>
            )}
          </div>
    ...
    <div className="rounded-xl border border-brand-100 bg-brand-50/30 p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => set("is_visible", !item.is_visible)} title={item.is_visible ? "إخفاء" : "إظهار"} className="rounded p-1 text-gray-500 hover:bg-brand-100">
            {item.is_visible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
          </button>
          <button type="button" onClick={() => set("is_featured", !item.is_featured)} title="مميز" className={cn("rounded p-1", item.is_featured ? "text-amber-500" : "text-gray-400 hover:bg-brand-100")}>
      ...
      <div className="grid gap-3">
        ...
        {isImageKind(kind) && (
          <>
            <Field label="الصورة"><ImageUpload value={item.url ?? ""} onChange={(u) => set("url", u)} folder="projects" /></Field>
