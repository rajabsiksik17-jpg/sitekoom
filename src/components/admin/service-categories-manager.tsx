  ...
  return (
    <div>
      <PageTitle title="تصنيفات الخدمات" description="التصنيفات الرئيسية للخدمات (مثل البرمجة والتسويق)."
        action={<button type="button" onClick={() => setEditing({ name_ar: "", name_en: "", slug: "", icon: "sparkles", description_ar: "", description_en: "", sort: items.length, is_active: true })} className="btn-primary px-4 py-2.5"><Plus className="h-4 w-4" /> إضافة تصنيف</button>} />

      {items.length === 0 ? (
        <EmptyState title="لا توجد تصنيفات" action={<button type="button" onClick={() => setEditing({ name_ar: "", name_en: "", slug: "", icon: "sparkles", description_ar: "", description_en: "", sort: 0, is_active: true })} className="btn-primary px-4 py-2.5"><Plus className="h-4 w-4" /> إضافة</button>} />
      ) : (
