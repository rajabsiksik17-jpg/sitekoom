...
export function ProjectCard({
  ...
  const short = localize(
    locale,
    project.short_desc_ar,
  ...
  const previewOn = Boolean(
    preview?.enabled && screenshot
  );

  const mode = preview?.mode ?? "hover";

  const hoverDelayMs =
    (preview?.hoverDelay ?? 3) * 1000;

  // Per-device scroll speed (px/s) selected by viewport width;
  // constant regardless of screenshot height.
  const currentScrollSpeed = useMemo(() => {
    if (typeof window === "undefined") {
      return preview?.scrollSpeed ?? 240;
    }
  ...
  const [touchPreviewed, setTouchPreviewed] =
    useState(false);
  ...
  function onMediaClick(
    ...
    if (!previewOn) {
      // No preview → allow Link navigation
      return;
    }
  ...
  return (
    ...
    >
      <div
        className="relative aspect-square w-full overflow-hidden bg-brand-50"
        onPointerDown={
          onMediaPointerDown
        }
      ...
      >
        ...
        {project.logo ? (
          ...
          <div
            className={cn(
              "absolute inset-0 flex items-center justify-center bg-brand-gradient text-white transition-opacity duration-500",
              previewing
                ? "opacity-0"
                : "opacity-100"
            )}
        ...
        {badge && (
          <span
            className={cn(
              "absolute top-3 start-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-brand-700 backdrop-blur transition-opacity duration-300",
        ...
        {project.logo && (
          <span
            className={cn(
              "absolute left-1/2 top-1/2 aspect-square w-[42.74%] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full border-2 border-brand-300 bg-white/95 shadow-glow ring-4 ring-brand-500/10 backdrop-blur-md transition-all duration-500 group-hover:border-brand-400 group-hover:ring-brand-500/20 group-hover:scale-[1.03]",
              previewing &&
                "scale-90 opacity-0"
            )}
          >
            {/* 
              IMPORTANT:
              object-contain keeps the entire logo visible
              regardless of its original dimensions.
              
              The logo is fitted inside the circular
              container instead of being cropped.
            */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={project.logo}
          ...
          >
            ...
            <img
              ...
              style={
                ...
                !reducedMotion.current
                  ? {
                      animation: `preview-scroll ${scrollDuration}s ease-in-out infinite alternate`,
                      objectFit: "cover",
                    }
                  : undefined
              }
        ...
        {previewOn &&
          ...
          !previewing && (
            <span className="pointer-events-none absolute bottom-3 start-3 hidden rounded-full border border-white/30 bg-ink-900/50 px-3 py-1 text-[10px] font-medium text-white/80 backdrop-blur-md lg:block">
              {locale === "ar"
                ? `مرر الفأرة ${
                    preview?.hoverDelay ?? 3
                  } ثوانٍ لمعاينة الموقع`
                : `Hover for ${
                    preview?.hoverDelay ?? 3
                  } seconds to preview`}
            </span>
