...
import { getAppointmentSettings, overlaps } from "@/lib/appointments";
import { sendAppointmentCustomerEmail, getAppointmentServices, formatRange } from "@/lib/appointment-emails";

const actionSchema = z.object({
  action: z.enum(["approve", "reject", "reschedule", "set_status"]),
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.action === "approve") {
    ...
    const start = new Date(`${appt.requested_date}T${appt.requested_time}:00+03:00`);
    const end = new Date(start.getTime() + (appt.duration_minutes || settings.duration_minutes) * 60000);
  ...
  if (body.action === "reschedule") {
    ...
    const settings = await getAppointmentSettings();
    const proposedStart = new Date(`${body.new_date}T${body.new_time}:00+03:00`);
    const proposedEnd = new Date(proposedStart.getTime() + (appt.duration_minutes || settings.duration_minutes) * 60000);
    ...
    const confirmUrl = `${SITE_URL}/${locale === "ar" ? "ar" : "en"}/appointment/confirm?id=${appt.id}&token=${token}`;
    const rows = [
      [locale === "en" ? "Previous time" : "الوقت السابق", formatRange(appt.start_at, appt.end_at)],
      [locale === "en" ? "New proposed time" : "الموعد الجديد المقترح", formatRange(proposedStart.toISOString(), proposedEnd.toISOString())],
      [locale === "en" ? "Reason" : "السبب", body.reschedule_reason || ""],
    ];
    const bodyHtml = `<p style="margin:0 0 16px;color:#4b5563;">${locale === "en" ? "A new time has been proposed for your appointment. Please confirm it." : "تم اقتراح موعد جديد لحجزك. يرجى تأكيد الموعد الجديد."}</p><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">${rows
      .filter(([, v]) => v)
      .map(([k, v]) => `<tr><td style="padding:8px 0;color:#9ca3af;width:150px;vertical-align:top;font-size:13px;">${String(k)}</td><td style="padding:8px 0;color:#1f2937;font-weight:500;">${String(v)}</td></tr>`)
      .join("")}</table><div style="margin-top:20px;"><a href="${confirmUrl}" style="display:inline-block;background:linear-gradient(135deg,#7a1aff,#9d72ff);color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:10px;font-weight:700;">${locale === "en" ? "Approve new time" : "الموافقة على الموعد الجديد"}</a></div>`;
    await sendAppointmentCustomerEmail(appt as never, services, {
  ...
  if (body.action === "set_status") {
    ...
    const { error } = await admin.from("appointments").update({ status: body.status }).eq("id", appt.id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true });
  }
